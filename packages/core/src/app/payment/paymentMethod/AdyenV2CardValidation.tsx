...
import React, { useContext, useEffect, useMemo, useState, FunctionComponent } from 'react';
...
const AdyenV2CardValidation: FunctionComponent<AdyenV2CardValidationProps> = ({
    ...
    const initialValidationState = useMemo(() => getInitialValidationState({shouldShowNumberField, method: paymentMethod.method}), [shouldShowNumberField, paymentMethod]);

    const [ fieldsValidation, setFieldsValidation ] = useState<FieldsValidation>((initialValidationState));
    ...
    useEffect(() => {
        if (selectedInstrument?.bigpayToken) {
            setFieldsValidation(initialValidationState);
        } else {
            paymentContext?.disableSubmit(paymentMethod, false);
        }
    }, [selectedInstrument, paymentContext, initialValidationState]);
